...
import { Browser, BrowserContext, Page } from 'playwright-firefox';
...
export class BestBuy {
  ...
  public async addToCart(product: ProductInformation) {
    ...
    await Promise.all([
      sendDiscordMessage({ message: `Product "${productName}" added to cart!` }),
    ]);
  }
  ...
  private async checkout(retrying: boolean = false) {
    ...
    await Promise.all([
      sendDiscordMessage({ message: `Attempting checkout` }),
    ]);
    ...
    try {
      ...
      await Promise.all([
        sendDiscordMessage({ message: `Checkout did not went through, trying again` }),
      ]);
  ...
  private async changePostalCode(postalCode: string) {
    ...
    await page.waitForSelector('#postalCode', { timeout: 50000 });
  ...
  private async submitGuestOrder() {
    ...
    await Promise.all([
      sendDiscordMessage({ message: `Placing order...` }),
    ]);
    ...
    await Promise.all([
      sendDiscordMessage({ message: `Order placed!` }),
    ]);
